...
1. General pre-trained LLMs generating raw ASP can achieve high syntactic accuracy (compiles, runs), but inconsistent  semantic accuracy (wrong answer sets). Fine-tuning can solve for this, but is out of project's scope. Therefore, I will not be letting the LLM generate raw ASP. It will instead be filling in a structured schema with a deterministic compiler (both written by me based on intents) narrowing risk to schema-field selection (and therefore making it easier to catch errors in translation). While AMR and CNL are both valid intermediaries, a schema was ultimately chosen because AMR is general purpose with no specific knowledge of Gemini's specific predicate vocaburary, needing a general to specific mapping regardless, adding complexity with no clear benefit for this project. CNL was also considered, but its restricted vocabulary and grammar run counter to my goal of exploring how unconstrained natural language can be translated into game mechanics and design intent, and would also again require a translation layer to Gemini's specific predicate vocabuary.
2. Existing pipeline translation (fact→rule→constraint→assembly) has no mechanisms to catch errors introduced early and the AMR parsing mistakes directly are represented onto wrong constraints with nothing to stop them. Hence I chose a solver-feedback repair loop over one-shot translation specifically so clingo's actual output could be the check with UNSAT triggering a loop back to the LLM for revision (with the use of `debug_rules.py`'s binary-search localization and a provenance map to point at a specific schema field if possible).
3.  LLMs struggle to construct complete correct answer sets(specifically when facts change, but logic remains the same), especially on complex/real-world programs but perform relatively better on direct logical reasoning/classification-style tasks. Therefore, I'm ensuring that the LLM never computes an answer set with its role (schema-filling, and in the repair loop, schema revision given a specific error) being deliberately closer to extraction and verification-style reasoning than to open-ended answer-set generation, using its strengths.
4.  Open gap: none of the three papers address fuzzy/creative intent (only specific subsets, like generic programming patterns, bounded combinatorial puzzles, benchmark ASP problems). Game design inherently is open-ended and because of that, has no single right answer to evaluate the ability of the generator without subjective human viewpoints. UNSAT can catch structural errors, but ultimately cannot tell whether the resulting program follows the human intentions. This can be partially addressed by Gemini's own bidirectional proceduralist-reading analysis (which will be used in the pipeline) and the existing human-evaluation protocol but there is no automated fix for prompt to schema mistranslation.
